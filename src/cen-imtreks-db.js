...
function resolveCenImtreksDbPath(dbPath, appDataPath) {
  const normalized = String(dbPath || "").trim();
  return normalized || getDefaultCenImtreksDbPath(appDataPath);
}
...
async function ensureSchema(client) {
  await client.executeMultiple(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS "${PROJECT_TABLE_NAME}_projectNameKey_key"
      ON "${PROJECT_TABLE_NAME}" ("projectNameKey");
    CREATE INDEX IF NOT EXISTS "${PROJECT_TABLE_NAME}_updatedAt_idx"
      ON "${PROJECT_TABLE_NAME}" ("updatedAt" DESC);
  `);
}
...
function deriveProjectName(state = {}) {
  ...
  const fallback =
    ...
    "Projekt CEN IMTREKS";
  return fallback || "Projekt CEN IMTREKS";
}
...
async function saveProjectState(dbPath, state, options = {}) {
  ...
  return withDb(dbPath, async (client) => {
    ...
    if (currentRow && duplicateByName && Number(currentRow.id) !== Number(duplicateByName.id)) {
      throw new Error(`Projekt "${normalizedState.projectName}" juz istnieje.`);
    }

    const targetRow = currentRow || (!createOnly ? duplicateByName : null);
    const rowCount = countProjectRows(normalizedState);
